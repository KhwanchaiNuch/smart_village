    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm px-5 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {scopeBanner.subtitle && (
              <p className="text-xs text-gray-400">{scopeBanner.subtitle}</p>
            )}
          </div>
        </div>
        {scopeBanner.badge && (
      ...
      <div className="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
        ...
        >
          ...
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 text-gray-400 transition-transform duration-200 ${showAiSummary ? "rotate-180" : ""}`}
              ...
              {allKpi.map((card) => {
                ...
                return (
                  <div key={card.label} className={`rounded-xl border border-gray-100 dark:border-gray-800 p-4 text-center relative ${card.mock ? "opacity-70" : ""}`}>
                    {card.mock && <span className="absolute top-1.5 right-1.5 text-[9px] bg-gray-100 dark:bg-gray-800 text-gray-400 rounded px-1">mock</span>}
