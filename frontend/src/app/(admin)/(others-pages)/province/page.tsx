...
export default function ProvincePage() {
  ...
  const delItem = async (p: Province) => {
    const r = await Swal.fire({ icon: "warning", title: "ยืนยันการลบ?", html: `ลบ <b>${p.nameTh}</b> และข้อมูลย่อยที่เกี่ยวข้อง`,
    ...
    try { await axios.delete(`/provinces/${p.provinceId}`); await fetchData(); ok(); } catch (e) { err(e); }
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    if (!r.isConfirmed) return;
    setLoading(true);
    try { await Promise.allSettled(selectedIds.map((id) => axios.delete(`/provinces/${id}`))); await fetchData(); ok(); }
                    ...
                    <TableRow key={p.provinceId} className={selectedIds.includes(p.provinceId) ? "bg-blue-50 dark:bg-blue-500/10" : ""}>
                      ...
                      <TableCell className="px-4 py-3 text-center">
                        <div className="flex items-center justify-center gap-2">
                          <button onClick={() => editItem(p)} className="flex h-9 w-9 items-center justify-center rounded-full border border-yellow-500 bg-yellow-500 text-white hover:bg-yellow-600">
                          ...
                          <button onClick={() => delItem(p)} className="flex h-9 w-9 items-center justify-center rounded-full border border-red-600 bg-red-600 text-white hover:bg-red-700">
