...
export default function HealthRecordPage() {
	...
	const filtered = tableData.filter((r) => {
		...
		return (
			...
			(r.checkDate || "").includes(q) ||
			(r.bp || "").toLowerCase().includes(q) ||
			(r.riskGroup || "").toLowerCase().includes(q)
		);
	});
	...
	const handleDeleteSelected = async () => {
		...
		const result = await Swal.fire({
			icon: "warning",
			title: "ยืนยันการลบ?",
			html: `คุณกำลังจะลบข้อมูลสุขภาพจำนวน <b>${selectedIds.length}</b> รายการ`,
		...
		try {
			...
			const responses = await Promise.allSettled(
				selectedIds.map((id) => axios.delete(`/health-records/${id}`))
			);
			...
			<ComponentCard title=''>
				<div className="flex items-center justify-between mb-4">
					<h3 className="text-lg font-semibold text-gray-800 dark:text-white">
						สุขภาพเชิงตัวเลข (Health Record)
					</h3>
							...
							<Table>
								<TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
									<TableRow>
										...
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">วันที่ตรวจ</TableCell>
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">ความดันโลหิต</TableCell>
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">น้ำตาล (mg/dL)</TableCell>
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">BMI</TableCell>
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">กลุ่มเสี่ยง</TableCell>
										<TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-center text-theme-xs dark:text-gray-400">เยี่ยมบ้าน</TableCell>
								...
								<TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
									{filtered.map((record) => {
										...
										return (
											<TableRow key={record.id} className={isSelected ? "bg-red-50 dark:bg-red-500/10" : ""}>
												<TableCell className="px-4 py-3 text-center">
													<Checkbox
														checked={isSelected}
														onChange={(checked) => toggleSelectOne(record.id, checked)}
													/>
												</TableCell>
												...
												<TableCell className="px-4 py-3 text-gray-500 text-center text-theme-sm dark:text-gray-400">
													<Badge
														size="sm"
														color={record.needHomeVisit === true ? "error" : record.needHomeVisit === false ? "success" : "warning"}
													>
														{record.needHomeVisit === true ? "ต้องการ" : record.needHomeVisit === false ? "ไม่ต้องการ" : "ไม่มีข้อมูล"}
													</Badge>
												</TableCell>
												<TableCell className="px-4 py-3 text-center">
													{canEdit("/healthrecord") && (
													<a
														href={`/healthrecord/edit?id=${record.id}`}
