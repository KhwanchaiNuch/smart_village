...
export default function VillagerAddPerson() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "บันทึกสำเร็จ",
        text: "เพิ่มสมาชิกในครัวเรือนเรียบร้อยแล้ว",
        timer: 1800,
  ...
  return (
    <>
      <ComponentCard title="เพิ่มสมาชิกในครัวเรือน ( Add Person )">

        {/* แสดงครัวเรือนที่ถูก lock ไว้ — ลูกบ้านไม่สามารถเปลี่ยนได้ */}
        <div className="rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 px-4 py-3 flex items-center gap-2">
          <span className="text-blue-500 text-lg">🏠</span>
          <p className="text-sm text-blue-700 dark:text-blue-300">
            เพิ่มสมาชิกในครัวเรือนรหัส <strong>{householdId}</strong>
          </p>
        </div>

        {/* แถว 1: เลขบัตรประชาชน | คำนำหน้า | ชื่อ */}
        ...
        {/* แถว 2: นามสกุล | อายุ | (ว่าง — align กับ form เดิม) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <div>
            <Label>นามสกุล *</Label>
        ...
        {/* แถว 3: เพศ | วันเกิด | มีทะเบียนบ้านหรือไม่ */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        {/* แถว 4: สถานะสมรส | ระดับการศึกษา | อาชีพหลัก */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <div>
        ...
        {/* แถว 5: อาชีพรอง | รายได้/เดือน | รายการโรคประจำตัว */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <div>
        ...
        {/* แถว 6: ป่วย/มีโรคเรื้อรัง | ติดเตียง | ผู้พิการ */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button
            onClick={() => router.push("/villager")}
            disabled={loading}
            className="px-6 py-2 border border-gray-400 text-gray-600 rounded-full text-sm font-medium hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-800"
          >
            ยกเลิก
          </button>
        </div>
