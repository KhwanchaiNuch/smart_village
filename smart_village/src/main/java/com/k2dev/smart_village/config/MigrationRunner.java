    ...
    private void runMigrations() {
        ...
        try {
            jdbc.execute("CREATE TABLE IF NOT EXISTS community_issue_log (" +
                ...
                "detail TEXT, status VARCHAR(50), remark1 VARCHAR(500), remark2 VARCHAR(500)," +
                "remark3 VARCHAR(500), remark4 VARCHAR(500), remark5 VARCHAR(500)," +
                "created_at TIMESTAMP DEFAULT NOW())");
            jdbc.execute("ALTER TABLE community_issue_log ADD COLUMN IF NOT EXISTS image_url TEXT");
        } catch (Exception ignored) {}
            ...
            for (Integer sid : missing) {
                try {
                    ...
                    try {
                        ...
                        System.out.println("[MigrationRunner] Created village (no tambon_id) village_id=" + sid);
                    } catch (Exception e3) {
